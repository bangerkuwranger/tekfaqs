function gpTabMaker(h, i, f) {
    ...
    this.buildSelectControl = function () {
        ...
        this.selectControl = jQuery( "<select></select>").attr( "id", "gp-tab-select").addClass(f ? f : "" );
    ...
    this.buildTextInputControl = function ( id ) {
    	...
    	var labelElement = '<label for="gp_tab_title">Tab ' + id + ' Title</label>';
    	var inputElement = '<input type="text" id="gp_tab_title_' + id.toString() + '" class="txt input-text" name="gp_tab_title" />';
    ...
    this.buildTabButtons = function (a) {
        if (this.buttonsControl) {
			...
			jQuery( 'label[for="gp_tab_title"]').parents( 'tr').remove();

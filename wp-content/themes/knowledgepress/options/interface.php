...
function guerillaframework_tabs() {
...
function guerillaframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		// Wrap all options
		if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) && ( $value['type'] != "title" ) ) {
			...
			if ( isset( $value['name'] ) ) {
				$output .= '<div class="heading">' . esc_html( $value['name'] ) . '</div>' . "\n";
			}
		...
		if ( ( $value['type'] != 'heading' ) && ( $value['type'] != 'info') && ( $value['type'] != "section" ) && ( $value['type'] != "title") ) {
		...
		case "color":
			...
			elseif ( ! preg_match( '/^#' . $hex_color . '/i', $meta ) ) // Value doesn't match #123abc, so sanitize to just #.
				$meta = "#";
			$output .= '<input class="cmb_colorpicker cmb_text_small" type="text" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" id="' . esc_attr( $value['id'] ) . '" value="' . $meta . '" />';
			break;

		// Uploader
		case "upload":
			$output .= guerillaframework_medialibrary_uploader( $value['id'], $val, null );
			break;
		...
		case 'typography':
			...
			$typography_options = array(
				'color' => true,
				'sizes' => gt_recognized_font_sizes(),
				'faces' => gt_recognized_font_faces(),
				'styles' => gt_recognized_font_styles(),
			);
			...
			// Font Color
			$font_color = '';
			$meta = esc_attr( $typography_stored['color'] );
			$hex_color = '(([a-fA-F0-9]){3}){1,2}$';
			if ( preg_match( '/^' . $hex_color . '/i', $meta ) ) // Value is just 123abc, so prepend #.
				$meta = '#' . $meta;
			elseif ( ! preg_match( '/^#' . $hex_color . '/i', $meta ) ) // Value doesn't match #123abc, so sanitize to just #.
			...
			if ( $typography_options['color'] ) {
				...
				$font_color .= '<input class="cmb_colorpicker cmb_text_small" name="' . esc_attr( $option_name . '[' . $value['id'] . '][color]' ) . '" id="' . esc_attr( $value['id'] . '_color' ) . '" type="text" value="' . $meta . '" />';
			}
			...
			$typography_fields = apply_filters( 'gt_typography_fields', $typography_fields, $typography_stored, $option_name, $value );
		...
		case 'background':
			...
			$output .= guerillaframework_medialibrary_uploader( $value['id'], $background['image'], null, '',0,'image');
			...
			$repeats = gt_recognized_background_repeat();
			...
			$positions = gt_recognized_background_position();
			...
			$attachments = gt_recognized_background_attachment();
		...
		case "info":
			...
			if ( $value['desc'] ) {
				$output .= '<div class="explain-info">' . apply_filters('gt_sanitize_info', $value['desc'] ) . '</div>' . "\n";
			}
			$output .= '</div>' . "\n";
			break;
		...
		//	Heading Subtitle & Description
			if ( isset($value['title']) ) {
				...
				$output .= '<div ' . $id . 'class="' . esc_attr( $class ) . '">' . "\n";
				$output .= '<h3>' . esc_html( $value['title'] ) . '</h3>' . "\n";
				if ( isset($value['desc'] ) ) {
					$output .= apply_filters('gt_sanitize_info', $value['desc'] ) . "\n";
				}
				$output .= '</div>' . "\n";
			} else {
				$output .= '<div class="heading-notitle"></div>' . "\n";
			}
			break;

...
Description: Generator based on WooThemes shortcode generator.
...
-----------------------------------------------------------------------------------*/
	...
	function init() {
		...
		if ( ( current_user_can( 'edit_posts' ) || current_user_can( 'edit_pages' ) ) && get_user_option( 'rich_editing') == 'true' && ( in_array( $pagenow, array( 'post.php', 'post-new.php', 'page-new.php', 'page.php' ) ) ) )  {
			...
			wp_register_script( 'woo-colourpicker', get_stylesheet_directory_uri() . '/framework/shortcodes/shortcode-generator/js/colorpicker.js', array( 'jquery' ), '3.6', true ); // Loaded into the footer.
			wp_enqueue_script( 'woo-colourpicker' );
			
			// Register the colourpicker CSS.
			wp_register_style( 'woo-colourpicker', get_stylesheet_directory_uri() . '/framework/shortcodes/shortcode-generator/css/colorpicker.css' );
			wp_enqueue_style( 'woo-colourpicker' );
			
			// Register the custom CSS styles.
			wp_register_style( 'woo-shortcode-generator', get_stylesheet_directory_uri() . '/framework/shortcodes/shortcode-generator/css/shortcode-generator.css' );
...
-----------------------------------------------------------------------------------*/
	...
	function filter_mce_external_plugins( $plugins ) {
        ...
        $plugins['WooThemesShortcodes'] = get_stylesheet_directory_uri() . '/framework/shortcodes/shortcode-generator/js/shortcode-generator/editor_plugin.js';
...
function framework_url() {
	...
	return trailingslashit( get_stylesheet_directory_uri() . '/' . basename( dirname( __FILE__ ) ) );
